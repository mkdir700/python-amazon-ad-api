...
class Client(BaseClient):
    ...
    def _download(self, params: dict = None, headers=None) -> ApiResponse:
        ...
        try:
            ...
            next_token = None
            return ApiResponse(error, next_token, headers=self.headers, response=e.response)
        ...
        if mode == "raw":
            ...
            next_token = None
            return ApiResponse(bytes, next_token, headers=r.headers, response=r)
        
        elif mode == "url":
            
            next_token = None
            return ApiResponse(r.url, next_token, headers=r.headers, response=r)
            ...
            if bytes[0:2] == b'\x1f\x8b':
                ...
                read_data = f.read()
                next_token = None
                return ApiResponse(json.loads(read_data.decode('utf-8')), next_token, headers=r.headers, response=r)
            
            else:
                logging.info("Is bytes snapshot")
                next_token = None
                return ApiResponse(json.loads(r.text), next_token, headers=r.headers, response=r)
            ...
            if bytes[0:2] == b'\x1f\x8b':
                ...
                fo.close()
                next_token = None
                return ApiResponse(name + ".json", next_token, headers=r.headers, response=r)
            else:
                fo = open(name + ".json", 'w')
                fo.write(r.text)
                fo.close()
            ...
            next_token = None
            return ApiResponse(name + ".json.gz", next_token, headers=r.headers, response=r)
            ...
            next_token = None
            return ApiResponse(name + ".zip", next_token, headers=r.headers, response=r)
            ...
            next_token = None
            return ApiResponse(error, next_token, headers=self.headers, response=r)
    ...
    def _check_response(res) -> ApiResponse:
        ...
        return ApiResponse(data, next_token, headers=headers, response=res)
